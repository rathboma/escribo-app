...
const REPO = 'rathboma/escribo-app'
...
// (escribo-web: _data/downloads.yml suffixes + assets/js/download.js).
function assetEntry(asset, urlBase) {
  const name = asset.name
  const extension = name.split('.').pop()

  const arch = name.includes('arm64') || name.includes('aarch64') ? 'arm64'
    : name.includes('universal') ? 'universal'
    : 'x86_64'

  const type = name.includes('portable') ? 'portable' : 'installer'
...
function main() {
  ...
  try {
    ...
    // electron-builder update metadata (*.yml, *.blockmap) is mirrored above
    // but excluded from the manifest - it is not a user-facing download.
    const manifest = {
